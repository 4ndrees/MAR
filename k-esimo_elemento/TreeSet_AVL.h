...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1, int iz = 0, int dr = 0) : elem(e), iz(i), dr(d), altura(alt), tam_i(iz), tam_d(dr) {}
    };
...
public:
    ...
    T const& kesimo(int k) const {
        ...
        if (k <= 0 || k > this->nelems)
            throw std::out_of_range("Fuera de rango");

        int posicion = k;
        Link nodoActual = raiz;

        while (nodoActual != nullptr) {
           
            if (posicion == nodoActual->tam_i) {
                return nodoActual->elem;
            }
            else if (posicion < nodoActual->tam_i) {
                nodoActual = nodoActual->iz;
            }
            else {
                posicion -= nodoActual->tam_i;
                nodoActual = nodoActual->dr;
            }
        }
